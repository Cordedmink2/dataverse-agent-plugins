...
# pattern -> schema filename. Charts (<visualization>), forms (<forms>) AND saved queries
# (<savedqueries>) have no association on purpose: they are pac WRAPPER files whose root the XSD
# doesn't declare, so whole-file validation would only produce a false error on the wrapper root.
# The validator (Validate-DataverseXml.ps1) owns them via per-fragment extraction (systemform/form,
# datadescription/datadefinition, savedquery). Kept in sync with the associations the shim
# (scripts/lsp-launch.mjs) injects for Claude Code.
$assoc = [ordered]@{
