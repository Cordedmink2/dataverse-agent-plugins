...
// pattern -> XSD filename. Charts (<visualization>), forms (<forms>) AND saved queries
// (<savedqueries>) have no association on purpose: they are pac WRAPPER files whose root the XSD
// doesn't declare, so lemminx would false-positive on the wrapper root. The validator
// (Validate-DataverseXml.ps1) owns them, via per-fragment extraction (systemform/form,
// datadescription/datadefinition, savedquery).
const assoc = {
